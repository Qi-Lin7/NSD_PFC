...
# Load the betas
if end_ind < len(share_imgs_list):
    betas_array = Parallel(n_jobs=14)(
        delayed(load_and_mask_native)(i, curr_sub, all_seq[:, :len(curr_sub_df_filtered)], 
                              lh_frontal_mask,
                              rh_frontal_mask,
                              betas_dir) for i in share_imgs_list[start_ind:end_ind])
    ...
    betas_array = Parallel(n_jobs=14)(
        delayed(load_and_mask_native)(i, curr_sub, all_seq[:, :len(curr_sub_df_filtered)], 
                              lh_frontal_mask,
                              rh_frontal_mask,
                              betas_dir) for i in share_imgs_list[start_ind:])
